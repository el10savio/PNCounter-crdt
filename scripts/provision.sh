...
# of the pncounter peers
declare -a peers=()
declare -a peer_id_list=()

# Name of the PNCounter cluster network
...
echo "Provisioning PNCounter Docker Cluster"

echo "Building PNCounter Docker Image"
docker build -t pncounter -f Dockerfile .

if [[ $? -ne 0 ]]; then
    echo "Unable To Build PNCounter Docker Image"
    exit 255
fi

echo "Building PNCounter Cluster Network"
...
for peer_index in "${!peers[@]}"; do
    docker run -p "${peers[$peer_index]}":8080 --net $network -e "PEERS="$comma_separated_peer_id_list"" -e "MY_NODE="${peers[$peer_index]}"" -e "NETWORK="$network"" --name="peer-$peer_index" -d pncounter
done
